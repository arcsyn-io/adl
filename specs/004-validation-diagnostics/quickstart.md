# Quickstart Validation: Validação semântica e diagnósticos
...
1. Executar `pnpm --filter @adl/diagnostics test`.
...
Todos os comandos devem ser executados no serviço Docker `workspace`, por exemplo:
`docker compose exec workspace bash -lc "pnpm --filter @adl/diagnostics test"`.
