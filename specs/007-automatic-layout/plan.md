# Implementation Plan: Layout automático

**Branch**: `codex/adl-plans-tasks` | **Date**: 2026-06-29 | **Spec**: [spec.md](./spec.md)

## Summary

Entregar a feature em `packages/adl-layout`, com API pública `@adl/layout`, respeitando as dependências do roadmap e mantendo regras de domínio fora de componentes React.

## Technical Context

**Language/Version**: TypeScript 6, módulos ES
**Primary Dependencies**: @adl/semantic, @adl/renderer; decisão: ELK encapsulado e saída normalizada
...
**Scale/Scope**: feature 007, três histórias entregáveis
...
```text
specs/007-automatic-layout/
├── spec.md
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
├── contracts/layout-contract.md
└── tasks.md
...
**Structure Decision**: `@adl/layout` expõe contrato mínimo. Integrações de UI são adaptadores finos e dependências apontam somente para camadas anteriores.

## Phase 0: Research

Research.md registra ELK encapsulado e saída normalizada, alternativas, determinismo, erros, acessibilidade aplicável e performance. Não restam NEEDS CLARIFICATION.

## Phase 1: Design & Contracts

- Modelar LayoutResult, LayoutOptions, NodeGeometry, EdgeRoute em data-model.md.
- Definir `contracts/layout-contract.md`.
- Validar cenários com quickstart.md.
- Limitar implementação futura a `packages/adl-layout` e testes associados.

## Dependency and Delivery Strategy

**Requires**: @adl/semantic, @adl/renderer
**Provides**: @adl/layout
