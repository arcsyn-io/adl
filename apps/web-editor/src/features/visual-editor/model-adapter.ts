...
import type { VisualModel } from './commands.js'
...
export function fromDiagramModel(model: DiagramModel): VisualModel {
  return {
    ...
    relations: Object.fromEntries(model.relations.map(relation => [relation.identity.value, { id: relation.identity.value, sourceId: relation.source.identity.value, targetId: relation.target.identity.value, ...(relation.name ? { name: relation.name } : {}) }])),
...
export function toDiagramModel(model: VisualModel): DiagramModel {
  ...
  return {
    ...
    relations: Object.values(model.relations).map(relation => ({ identity: identity('relation', relation.id), source: reference(relation.sourceId), target: reference(relation.targetId), name: relation.name ?? null, type: null, description: null, properties: {}, provenance: provenance('Relation') })),
